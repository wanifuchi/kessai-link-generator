    ...
    return await withSession(
      ...
      async () => {
        ...
        const paymentConfig = await PaymentConfigService.getConfig(
          validatedData.userPaymentConfigId,
          session.user.id
        );

        if (!paymentConfig || paymentConfig.provider !== 'stripe' || !paymentConfig.isActive) {
          return NextResponse.json({
            success: false,
            error: '有効なStripe決済設定が見つかりません'
          }, { status: 400 });
        }

        // Stripe設定を取得
        const stripeConfig = paymentConfig.config as any;
          ...
          const savedPaymentLink = await prisma.paymentLink.create({
            data: {
              ...
              metadata: {
                ...
                environment: paymentConfig.isTestMode ? 'sandbox' : 'production',
